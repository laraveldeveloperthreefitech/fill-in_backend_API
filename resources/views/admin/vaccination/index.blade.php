                ...
                <div class="row flex-grow">
                  <div class="col-12 grid-margin stretch-card">
                    <div class="card card-rounded">
                      <div class="card-body">
                        <h4 class="card-title card-title-dash">Vaccination List</h4>
                        ...
                        <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-2">
                            ...
                            <form action="" class="d-flex flex-wrap align-items-center gap-2 mb-0">
                                ...
                                <a type="button" href="{{route('admin.vaccination')}}" class="btn btn-warning text-white clear-btn">Clear</a>
                            </form>

                            <!-- Right-aligned Buttons -->
                            <div class="d-flex flex-wrap gap-2 justify-content-end">
                                <button class="btn btn-outline-primary addvaccination" >Add</button>
                          ...
                          <table class="table select-table">
                            ...
                            <tbody id="indexData">
                              ...
                              <tr>
                                ...
                                <td>
                                  <div class="d-flex align-items-center">
                                  <a href="javascript:void(0);" class="btn btn-sm btn-primary me-2 editvaccination" 
                                      ...
                                      <i class="mdi mdi-pencil menu-icon"></i>
                                        ...
                                        @if($row->status)
                                        <a href="{{ route('vaccination.status', ['id' => $row->id]) }}" class="btn btn-sm btn-success text-white me-2" id="active_link-{{$row->id}}" title="Activate">
                                            <i id="active_ids-{{$row->id}}" class="mdi mdi-toggle-switch menu-icon"></i>
                                        </a>
                                        @else
                                        <a href="{{ route('vaccination.status', ['id' => $row->id]) }}" class="btn btn-sm btn-secondary text-white me-2" id="deactive_link-{{$row->id}}"  title="Activate">
                                       ...
                                       <a href="javascript:void(0);" 
                                                ...
                                                data-url="{{ route('vaccination.delete', ['id' => $row->id]) }}">
...
<div class="modal fade" id="vaccinationModal" tabindex="-1" aria-labelledby="vaccinationLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content cusomize-employment-model">
      ...
      <form id="addvaccinationForm" enctype="multipart/form-data">
        @csrf
...
<script>
   var module = 'vaccination Type';
   var ActivateUrl = "{{ route('vaccination.active') }}";
   var DeActivateUrl = "{{ route('vaccination.de-active') }}";
...
<script>
  $(document).ready(function () {
    ...
    let baseUrl = "{{config('filepaths.vaccination.public_url')}}";
   
    $(".addvaccination").on("click", function () {
        $('#addvaccinationForm')[0].reset();
        $("#vaccinationModal").modal("show");
        $('#specualizationLabel').html('Add vaccination Range');
    });

    // Open Edit Modal & Fetch Data
    $(".editvaccination").on("click", function () {
        $('#addvaccinationForm')[0].reset();
        let response = $(this).data("row");
        $("#vaccination_id").val(response.id);
        $("#name").val(response.name);
        $('#specualizationLabel').html('Edit vaccination Range');
        $("#vaccinationModal").modal("show");
    ...
    $("#addvaccinationForm").on("submit", function (e) {
        ...
        $.ajax({
            url: "{{route('vaccination.update')}}",  // Adjust your update route
            type: "POST",
            data: formData,
