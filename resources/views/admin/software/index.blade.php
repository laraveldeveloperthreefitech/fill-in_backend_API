                ...
                <div class="row flex-grow">
                  <div class="col-12 grid-margin stretch-card">
                    <div class="card card-rounded">
                      <div class="card-body">
                        <h4 class="card-title card-title-dash">Software List</h4>
                        ...
                        <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-2">
                            ...
                            <form action="" class="d-flex flex-wrap align-items-center gap-2 mb-0">
                                ...
                                <a type="button" href="{{route('admin.software')}}" class="btn btn-warning text-white clear-btn">Clear</a>
                            </form>

                            <!-- Right-aligned Buttons -->
                            <div class="d-flex flex-wrap gap-2 justify-content-end">
                                <button class="btn btn-outline-primary addsoftware" >Add</button>
                          ...
                          <table class="table select-table">
                            ...
                            <tbody id="indexData">
                              ...
                              <tr>
                                ...
                                <td>
                                  <div class="d-flex align-items-center">
                                  <a href="javascript:void(0);" class="btn btn-sm btn-primary me-2 editsoftware" 
                                      ...
                                      <i class="mdi mdi-pencil menu-icon"></i>
                                        ...
                                        @if($row->status)
                                        <a href="{{ route('software.status', ['id' => $row->id]) }}" class="btn btn-sm btn-success text-white me-2" id="active_link-{{$row->id}}" title="Activate">
                                            <i id="active_ids-{{$row->id}}" class="mdi mdi-toggle-switch menu-icon"></i>
                                        </a>
                                        @else
                                        <a href="{{ route('software.status', ['id' => $row->id]) }}" class="btn btn-sm btn-secondary text-white me-2" id="deactive_link-{{$row->id}}"  title="Activate">
                                       ...
                                       <a href="javascript:void(0);" 
                                                ...
                                                data-url="{{ route('software.delete', ['id' => $row->id]) }}">
...
<!-- Edit software Modal -->
<div class="modal fade" id="softwareModal" tabindex="-1" aria-labelledby="softwareLabel" aria-hidden="true">
    ...
    <div class="modal-content cusomize-employment-model">
      ...
      <form id="addsoftwareForm" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          <input type="hidden" name="id" id="software_id">
...
<script>
   var module = 'software Type';
   var ActivateUrl = "{{ route('software.active') }}";
   var DeActivateUrl = "{{ route('software.de-active') }}";
...
<script>
  $(document).ready(function () {
    ...
    let baseUrl = "{{config('filepaths.software.public_url')}}";
   
    $(".addsoftware").on("click", function () {
        $('#addsoftwareForm')[0].reset();
        $("#softwareModal").modal("show");
        $('#specualizationLabel').html('Add software Type');
    });

    // Open Edit Modal & Fetch Data
    $(".editsoftware").on("click", function () {
        $('#addsoftwareForm')[0].reset();
        let response = $(this).data("row");
        $("#software_id").val(response.id);
        $("#name").val(response.name);
        $('#specualizationLabel').html('Edit software Type');
        $("#softwareModal").modal("show");
    ...
    $("#addsoftwareForm").on("submit", function (e) {
        ...
        $.ajax({
            url: "{{route('software.update')}}",  // Adjust your update route
            type: "POST",
            data: formData,
            ...
            success: function (response) {
               if(response.status){
                  successMsg("software Updated successfully!");
