                ...
                <div class="row flex-grow">
                  <div class="col-12 grid-margin stretch-card">
                    <div class="card card-rounded">
                      <div class="card-body">
                        <h4 class="card-title card-title-dash">Practice Role List</h4>
                        ...
                        <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-2">
                            ...
                            <form action="" class="d-flex flex-wrap align-items-center gap-2 mb-0">
                                ...
                                <a type="button" href="{{route('admin.department')}}" class="btn btn-warning text-white clear-btn">Clear</a>
                            </form>

                            <!-- Right-aligned Buttons -->
                            <div class="d-flex flex-wrap gap-2 justify-content-end">
                                <button class="btn btn-outline-primary adddepartment" >Add</button>
                          ...
                          <table class="table select-table">
                            ...
                            <tbody id="indexData">
                              ...
                              <tr>
                                ...
                                <td>
                                  <div class="d-flex align-items-center">
                                  <a href="javascript:void(0);" class="btn btn-sm btn-primary text-white me-2 editdepartment" 
                                      ...
                                      <i class="mdi mdi-pencil menu-icon"></i>
                                        ...
                                        @if($row->status)
                                        <a href="{{ route('department.status', ['id' => $row->id]) }}" class="btn btn-sm btn-success text-white me-2" id="active_link-{{$row->id}}" title="Activate">
                                            <i id="active_ids-{{$row->id}}" class="mdi mdi-toggle-switch menu-icon"></i>
                                        </a>
                                        @else
                                        <a href="{{ route('department.status', ['id' => $row->id]) }}" class="btn btn-sm btn-secondary text-white me-2" id="deactive_link-{{$row->id}}"  title="Activate">
                                       ...
                                       <a href="javascript:void(0);" 
                                                ...
                                                data-url="{{ route('department.delete', ['id' => $row->id]) }}">
...
<div class="modal fade" id="departmentModal" tabindex="-1" aria-labelledby="departmentLabel" aria-hidden="true">
...
<script>
   ...
   var ActivateUrl = "{{ route('department.active') }}";
   var DeActivateUrl = "{{ route('department.de-active') }}";
...
<script>
  $(document).ready(function () {
    ...
    let baseUrl = "{{config('filepaths.department.public_url')}}";
   
    $(".adddepartment").on("click", function () {
        $('#adddepartmentForm')[0].reset();
        $("#departmentModal").modal("show");
        $('#specualizationLabel').html('Add Practice Role');
    });
    ...
    $(".editdepartment").on("click", function () {
        ...
        $('#specualizationLabel').html('Edit Practice Role');
        $("#departmentModal").modal("show");
    ...
    $("#adddepartmentForm").on("submit", function (e) {
        ...
        $.ajax({
            url: "{{route('department.update')}}",  // Adjust your update route
            type: "POST",
            data: formData,
