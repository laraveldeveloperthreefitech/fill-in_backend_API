                ...
                <div class="row flex-grow">
                  <div class="col-12 grid-margin stretch-card">
                    <div class="card card-rounded">
                      <div class="card-body">
                        <h4 class="card-title card-title-dash">Employment Type List</h4>
                        ...
                        <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-2">
                            ...
                            <form action="" class="d-flex flex-wrap align-items-center gap-2 mb-0">
                                ...
                                <a type="button" href="{{route('admin.employment')}}" class="btn btn-warning text-white clear-btn">Clear</a>
                            </form>

                            <!-- Right-aligned Buttons -->
                            <div class="d-flex flex-wrap gap-2 justify-content-end">
                                <button class="btn btn-outline-primary addEmployment" >Add</button>
                          ...
                          <table class="table select-table">
                            ...
                            <tbody id="indexData">
                              ...
                              <tr>
                                ...
                                <td>
                                  <div class="d-flex align-items-center">
                                  <a href="javascript:void(0);" class="btn btn-sm btn-primary me-2 editEmployment" 
                                      ...
                                      <i class="mdi mdi-pencil menu-icon"></i>
                                        ...
                                        @if($row->status)
                                        <a href="{{ route('employment.status', ['id' => $row->id]) }}" class="btn btn-sm btn-success text-white me-2" id="active_link-{{$row->id}}" title="Activate">
                                            <i id="active_ids-{{$row->id}}" class="mdi mdi-toggle-switch menu-icon"></i>
                                        </a>
                                        @else
                                        <a href="{{ route('employment.status', ['id' => $row->id]) }}" class="btn btn-sm btn-secondary text-white me-2" id="deactive_link-{{$row->id}}"  title="Activate">
                                       ...
                                       <a href="javascript:void(0);" 
                                                ...
                                                data-url="{{ route('employment.delete', ['id' => $row->id]) }}">
...
<div class="modal fade" id="employmentModal" tabindex="-1" aria-labelledby="employmentLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content cusomize-employment-model">
      ...
      <form id="addEmploymentForm" enctype="multipart/form-data">
        @csrf
...
<script>
   var module = 'Employment Type';
   var ActivateUrl = "{{ route('employment.active') }}";
   var DeActivateUrl = "{{ route('employment.de-active') }}";
...
<script>
  $(document).ready(function () {
    ...
    let baseUrl = "{{config('filepaths.employment.public_url')}}";
   
    $(".addEmployment").on("click", function () {
        $('#addEmploymentForm')[0].reset();
        $("#employmentModal").modal("show");
        $('#specualizationLabel').html('Add Employment Type');
    });
    ...
    $(".editEmployment").on("click", function () {
        ...
        $('#specualizationLabel').html('Edit Employment Type');
        $("#employmentModal").modal("show");
    ...
    $("#addEmploymentForm").on("submit", function (e) {
        ...
        $.ajax({
            url: "{{route('employment.update')}}",  // Adjust your update route
            type: "POST",
            data: formData,
            ...
            success: function (response) {
               if(response.status){
                  successMsg("employment Updated successfully!");
