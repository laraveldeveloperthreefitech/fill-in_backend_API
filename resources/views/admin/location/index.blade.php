                ...
                <div class="row flex-grow">
                  <div class="col-12 grid-margin stretch-card">
                    <div class="card card-rounded">
                      <div class="card-body">
                        <h4 class="card-title card-title-dash">Location Range List</h4>
                        ...
                        <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-2">
                            ...
                            <form action="" class="d-flex flex-wrap align-items-center gap-2 mb-0">
                                ...
                                <a type="button" href="{{route('admin.location')}}" class="btn btn-warning text-white clear-btn">Clear</a>
                            </form>

                            <!-- Right-aligned Buttons -->
                            <div class="d-flex flex-wrap gap-2 justify-content-end">
                                <button class="btn btn-outline-primary addlocation" >Add</button>
                          ...
                          <table class="table select-table">
                            ...
                            <tbody id="indexData">
                              ...
                              <tr>
                                ...
                                <td>
                                  <div class="d-flex align-items-center">
                                  <a href="javascript:void(0);" class="btn btn-sm btn-primary me-2 editlocation" 
                                      ...
                                      <i class="mdi mdi-pencil menu-icon"></i>
                                        ...
                                        @if($row->status)
                                        <a href="{{ route('location.status', ['id' => $row->id]) }}" class="btn btn-sm btn-success text-white me-2" id="active_link-{{$row->id}}" title="Activate">
                                            <i id="active_ids-{{$row->id}}" class="mdi mdi-toggle-switch menu-icon"></i>
                                        </a>
                                        @else
                                        <a href="{{ route('location.status', ['id' => $row->id]) }}" class="btn btn-sm btn-secondary text-white me-2" id="deactive_link-{{$row->id}}"  title="Activate">
                                       ...
                                       <a href="javascript:void(0);" 
                                                ...
                                                data-url="{{ route('location.delete', ['id' => $row->id]) }}">
...
<!-- Edit location Modal -->
<div class="modal fade" id="locationModal" tabindex="-1" aria-labelledby="locationLabel" aria-hidden="true">
...
<script>
   var module = 'location Type';
   var ActivateUrl = "{{ route('location.active') }}";
   var DeActivateUrl = "{{ route('location.de-active') }}";
...
<script>
  $(document).ready(function () {
    ...
    let baseUrl = "{{config('filepaths.location.public_url')}}";
   
    $(".addlocation").on("click", function () {
        $('#addlocationForm')[0].reset();
        $("#locationModal").modal("show");
        $('#specualizationLabel').html('Add Location Range');
    });
    ...
    $(".editlocation").on("click", function () {
        ...
        $('#specualizationLabel').html('Edit Location Range');
        $("#locationModal").modal("show");
    ...
    $("#addlocationForm").on("submit", function (e) {
        ...
        $.ajax({
            url: "{{route('location.update')}}",  // Adjust your update route
            type: "POST",
            data: formData,
