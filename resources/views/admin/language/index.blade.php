                ...
                <div class="row flex-grow">
                  <div class="col-12 grid-margin stretch-card">
                    <div class="card card-rounded">
                      <div class="card-body">
                        <h4 class="card-title card-title-dash">Language List</h4>
                        ...
                        <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-2">
                            ...
                            <form action="" class="d-flex flex-wrap align-items-center gap-2 mb-0">
                                ...
                                <a type="button" href="{{route('admin.language')}}" class="btn btn-warning text-white clear-btn">Clear</a>
                            </form>

                            <!-- Right-aligned Buttons -->
                            <div class="d-flex flex-wrap gap-2 justify-content-end">
                                <button class="btn btn-outline-primary addlanguage" >Add</button>
                          ...
                          <table class="table select-table">
                            ...
                            <tbody id="indexData">
                              ...
                              <tr>
                                ...
                                <td>
                                  <div class="d-flex align-items-center">
                                  <a href="javascript:void(0);" class="btn btn-sm btn-primary me-2 editlanguage" 
                                      ...
                                      <i class="mdi mdi-pencil menu-icon"></i>
                                        ...
                                        @if($row->status)
                                        <a href="{{ route('language.status', ['id' => $row->id]) }}" class="btn btn-sm btn-success text-white me-2" id="active_link-{{$row->id}}" title="Activate">
                                            <i id="active_ids-{{$row->id}}" class="mdi mdi-toggle-switch menu-icon"></i>
                                        </a>
                                        @else
                                        <a href="{{ route('language.status', ['id' => $row->id]) }}" class="btn btn-sm btn-secondary text-white me-2" id="deactive_link-{{$row->id}}"  title="Activate">
                                       ...
                                       <a href="javascript:void(0);" 
                                                ...
                                                data-url="{{ route('language.delete', ['id' => $row->id]) }}">
...
<!-- Edit language Modal -->
<div class="modal fade" id="languageModal" tabindex="-1" aria-labelledby="languageLabel" aria-hidden="true">
...
<script>
   var module = 'language Type';
   var ActivateUrl = "{{ route('language.active') }}";
   var DeActivateUrl = "{{ route('language.de-active') }}";
...
<script>
  $(document).ready(function () {
    ...
    let baseUrl = "{{config('filepaths.language.public_url')}}";
   
    $(".addlanguage").on("click", function () {
        $('#addlanguageForm')[0].reset();
        $("#languageModal").modal("show");
        $('#specualizationLabel').html('Add language Type');
    });
    ...
    $(".editlanguage").on("click", function () {
        ...
        $('#specualizationLabel').html('Edit language Type');
        $("#languageModal").modal("show");
    ...
    $("#addlanguageForm").on("submit", function (e) {
        ...
        $.ajax({
            url: "{{route('language.update')}}",  // Adjust your update route
            type: "POST",
            data: formData,
